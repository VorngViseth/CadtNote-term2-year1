...
bool moveCaptain(char gameGrid[5][5], char move){
    ...
    if (move == 'u' && rowC - 1 >= 0 && gameGrid[rowC - 1][columnC] != 'W') {
        gameGrid[rowC - 1][columnC] = 'C';
        gameGrid[rowC][columnC] = '0';
        canMove = true;

    } else if (move == 'd' && rowC + 1 <= 5 && gameGrid[rowC + 1][columnC] != 'W') {
        gameGrid[rowC + 1][columnC] = 'C';
        gameGrid[rowC][columnC] = '0';
        canMove = true;

    } else if (move == 'r' && columnC + 1 <= 5 && gameGrid[rowC][columnC + 1] != 'W') {
        gameGrid[rowC][columnC + 1] = 'C';
        gameGrid[rowC][columnC] = '0';
        canMove = true;

    } else if (move == 'l' && columnC - 1 >= 0 && gameGrid[rowC][columnC - 1] != 'W') {
        gameGrid[rowC][columnC + 1] = 'C';
        gameGrid[rowC][columnC] = '0';
        canMove = true;
    }
